...
static void
update_port_ids (struct pw_node *node)
{
  ...
  spa_list_for_each (inport, &this->input_ports, link) {
    ...
    spa_list_for_each (link, &inport->rt.links, rt.input_link) {
      ...
      *po = *pi;
      ...
      pw_log_trace ("node %p: process output %p %d", outport->node, po, po->buffer_id);
      ...
      if (res == SPA_RESULT_NEED_BUFFER) {
        ...
        *pi = *po;
        pw_log_trace ("node %p: have output %d %d", this, pi->status, pi->buffer_id);
        have_output = true;
      }
      else if (res < 0) {
...
static void
init_complete (struct pw_node *this)
{
  struct impl *impl = SPA_CONTAINER_OF (this, struct impl, this);
...
void
pw_node_set_data_loop (struct pw_node      *node,
 ...
 */
void
pw_node_destroy (struct pw_node * this)
{
  struct impl *impl = SPA_CONTAINER_OF (this, struct impl, this);
  ...
  pw_loop_invoke (this->data_loop->loop,
                  do_node_remove,
                  1,
                  0,
                  NULL,
 ...
 */
struct pw_port *
pw_node_get_free_port (struct pw_node     *node,
...
static void
node_activate (struct pw_node *this)
{
  struct pw_port *port;
                      ...
                      enum pw_node_state  state)
{
  ...
  pw_work_queue_add (impl->work,
                     node,
                     res,
                     (pw_work_func_t) on_state_complete,
 ...
 */
void
pw_node_update_state (struct pw_node     *node,
                      enum pw_node_state  state,
                      char               *error)
{
