...
static void bind_ctl_event(struct spa_source *source)
{
	...
	while ((err = snd_ctl_read(state->ctl, ev) > 0)) {
		...
		if (changed) {
			...
			// This might legitimately happen, for example on Capture/Playback Pitch updates
			spa_log_trace_fp(state->log, "Got a bind ctl wakeup event but nothing has changed: %u",
					state->ctl_pfds[0].revents);
		}
	}
