...
static void
clear_mems (struct pw_stream *stream)
{
...
static void
clear_buffers (struct pw_stream *stream)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static bool
stream_set_state (struct pw_stream      *stream,
...
static void
unhandle_socket (struct pw_stream *stream)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static void
add_node_update (struct pw_stream *stream, uint32_t change_mask)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static void
add_port_update (struct pw_stream *stream, uint32_t change_mask)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static inline void
send_need_input (struct pw_stream *stream)
{
#if 0
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  uint64_t cmd = 1;
...
static inline void
send_have_output (struct pw_stream *stream)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  uint64_t cmd = 1;
...
static void
add_request_clock_update (struct pw_stream *stream)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static void
do_node_init (struct pw_stream *stream)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static struct mem_id *
find_mem (struct pw_stream *stream, uint32_t id)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  struct mem_id *mid;
...
static struct buffer_id *
find_buffer (struct pw_stream *stream, uint32_t id)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
...
static inline void
reuse_buffer (struct pw_stream *stream, uint32_t id)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  struct buffer_id *bid;
...
static void
handle_socket (struct pw_stream *stream, int rtreadfd, int rtwritefd)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  ...
  impl->timeout_source = pw_loop_add_timer (stream->context->loop,
                                               on_timeout,
                                               stream);
  interval.tv_sec = 0;
  interval.tv_nsec = 100000000;
  pw_loop_update_timer (stream->context->loop,
                           impl->timeout_source,
                           NULL,
                           &interval,
                           false);
  ...
  if (SPA_COMMAND_TYPE (command) == context->type.command_node.Pause) {
    ...
    if (cu->body.flags.value & SPA_COMMAND_NODE_CLOCK_UPDATE_FLAG_LIVE) {
      ...
      pw_properties_setf (stream->properties,
                        "pipewire.latency.min", "%"PRId64, cu->body.latency.value);
    }
  ...
  for (i = 0; i < n_buffers; i++) {
    ...
    if (mid->ptr == NULL) {
      ...
      if (mid->ptr == MAP_FAILED) {
        mid->ptr = NULL;
        pw_log_warn ("Failed to mmap memory %d %p: %s", mid->size, mid, strerror (errno));
        continue;
      }
...
static void
on_node_proxy_destroy (struct pw_listener *listener,
                   ...
                   struct spa_format             **possible_formats)
{
  ...
  impl->direction = direction == PW_DIRECTION_INPUT ? SPA_DIRECTION_INPUT : SPA_DIRECTION_OUTPUT;
  impl->port_id = 0;
  impl->mode = mode;
...
bool
pw_stream_get_time (struct pw_stream *stream,
                       struct pw_time   *time)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  int64_t elapsed;
 ...
 */
bool
pw_stream_recycle_buffer (struct pw_stream *stream,
                          uint32_t          id)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
 ...
 */
bool
pw_stream_send_buffer (struct pw_stream     *stream,
                       uint32_t              id)
{
  struct stream *impl = SPA_CONTAINER_OF (stream, struct stream, this);
  struct buffer_id *bid;
  ...
  if (impl->trans->outputs[0].buffer_id != SPA_ID_INVALID) {
    pw_log_debug ("can't send %u, pending buffer %u", id, impl->trans->outputs[0].buffer_id);
    return false;
  }
