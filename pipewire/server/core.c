...
static void *
async_create_client_node_copy (struct pw_access_data *data, size_t size)
{
...
static void
async_create_client_node_free (struct pw_access_data *data)
{
...
static void
async_create_client_node_complete (struct pw_access_data *data)
{
  struct access_create_client_node *d = (struct access_create_client_node *) data;
  ...
  pw_client_node_notify_done (node->resource,
                              readfd,
                              writefd);
  goto done;
                         ...
                         uint32_t       new_id)
{
  ...
  for (i = 0; i < props->n_items; i++) {
    pw_properties_set (properties, props->items[i].key,
                                   props->items[i].value);
  }
...
struct pw_core *
pw_core_new (struct pw_main_loop  *main_loop,
             struct pw_properties *properties)
{
  struct impl *impl;
  struct pw_core *this;
  ...
  *global = this;
  ...
  spa_list_for_each (registry, &core->registry_resource_list, link)
    if (ACCESS_VIEW_GLOBAL (registry->client, this))
      pw_registry_notify_global (registry,
                                 this->id,
                                 type_name);
  ...
  spa_list_for_each (n, &core->node_list, link) {
    ...
    if (have_id) {
      if (n->global->id == id) {
        ...
        best = pw_node_get_free_port (n, pw_direction_reverse (other_port->direction));
        if (best)
          break;
      }
    } else {
...
pw_core_find_node_factory (struct pw_core *core,
                           const char     *name)
{
