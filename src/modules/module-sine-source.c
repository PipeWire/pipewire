...
/* SPDX-FileCopyrightText: Copyright © 2024 Arun Raghavan */
...
/** \page page_module_sine_source Sine Source
 *
 * A simple sine wave generator source.
 * source. We refer to the source code for more information.
 * ## Module Name
 *
 * `libpipewire-module-sine-source`
 *
 * ## Module Options
 *
 ...
 *\code{.unparsed}
 * context.modules = [
 * {   name = libpipewire-module-sine-source
 *     args = {
 *         node.name = "sine_source"
 *         node.description = "Sine source"
 *         stream.props = {
...
#define NAME "sine-source"

PW_LOG_TOPIC_STATIC(mod_topic, "mod." NAME);
#define PW_LOG_TOPIC_DEFAULT mod_topic

#define DEFAULT_FREQ 440
#define DEFAULT_AMPL 0.1 // -10 dB
...
#define MODULE_USAGE	"( node.latency=<latency as fraction> ) "				\
			...
			"( sine.frequency=<frequency, default:"SPA_STRINGIFY(DEFAULT_FREQ)"> ) "			\
			"( sine.amplitude=<amplited, default:"SPA_STRINGIFY(DEFAULT_AMPL)"> ) "			\
...
static const struct spa_dict_item module_props[] = {
	...
	{ PW_KEY_MODULE_DESCRIPTION, "A sine wave source" },
...
static void capture_stream_process(void *d)
{
	...
	for (unsigned int i = 0; i < size / impl->frame_size; i++) {
		double sample = sin(impl->pos) * impl->ampl;

		impl->pos += 2 * M_PI * impl->freq / impl->info.rate;
		if (impl->pos > 2 * M_PI)
			impl->pos -= 2 * M_PI;

		for (unsigned int j = 0; j < impl->info.channels; j++)
			data[i * impl->info.channels + j] = sample;
	}
...
static int create_stream(struct impl *impl)
{
	...
	impl->stream = pw_stream_new(impl->core, "sine source", impl->stream_props);
...
int pipewire__module_init(struct pw_impl_module *module, const char *args)
{
	...
	if (pw_properties_get(props, PW_KEY_NODE_NAME) == NULL)
		pw_properties_setf(props, PW_KEY_NODE_NAME, "sine-source-%u-%u", pid, id);
	...
	impl->freq = pw_properties_get_uint32(props, "sine.frequency", DEFAULT_FREQ);
	if ((str = pw_properties_get(props, "sine.amplitude")) != NULL)
		impl->ampl = pw_properties_parse_double(str);
	else
		impl->ampl = DEFAULT_AMPL;
	impl->pos = 0;
